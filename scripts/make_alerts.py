...
def main(a):
    ...
    if changes.empty:
        raise SystemExit("No polygons in changes_geojson.")
    changes = changes.to_crs("EPSG:4326")

    # area_m2 present? if not, compute via local UTM
    ...
    # 2) Restricted zones overlay (optional)
    changes["in_restricted_zone"] = False
    if a.restricted_geojson and os.path.exists(a.restricted_geojson):
        rz = gpd.read_file(a.restricted_geojson)
        if not rz.empty:
            if rz.crs != changes.crs:
                rz = rz.to_crs(changes.crs)
            ...
            print("Restricted zones file is empty.")
    else:
        print("No restricted zones provided.")
    ...
    print("Mean prob (non-NaN) stats:",
          pd.Series([v for v in mean_probs if pd.notna(v)]).describe())

    # 4) Apply filters one by one and print counts
    q = changes.copy()
    before = len(q)
    ...
    # 5) Safe centroids (compute in projected CRS, then back to WGS84)
